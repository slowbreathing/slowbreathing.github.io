...
Again, for most part, this is almost identical to standard LSTM Backward Propagation which we went through in detail in [part-3]. So if you have got a good hang of it, only a few things change. But before we go on, refresh [DHX] and the complete [section][Backward propagation through LSTMCell(Step-2)] before you resume here. Here is what we have to calculate.
...
> * Dhx, Dh_next, dxt as usual and then <strong>dh_next_recurr</strong> which is passed back to MultiRNNCell to be applied to next layer.
...
> * Dhx, Dh_next, dxt as usual and then <strong>dh_next_recurr</strong> which is passed back to MultiRNNCell to be applied to next layer.
> * Complete listing can be found at [code-1] in particular the [MultiRNNCell "compute_gradient()"][code-2]. Pay careful attention to the weights shapes.
{%
    include image.html
...
> * [Bidirectional RNNs].
> * Simple 2 seperate cells fed in with the same sequence in opposite directions.
{%
    include image.html
...
> * [Bidirectional RNNs]
> * The State look identical but reversed, and that is because they have been initialized indentically for comparision purposes. <strong>In practise we don't do that</strong>.
{%
    include image.html
...
### Summary
Finally, that would be the complete documnentation of LSTMs inner workings. Maybe a comparision with RNNs for vanishing gradient improvement will complete it. But that apart, once we have a drop on LSTMs like we did, using them in more complex architectures will become a lot easier. In the immediate next article well look at the improvements in gradient propagation, if any, brought about by LSTMs over RNNs.   
